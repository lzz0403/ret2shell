...
import { cursorRestorePosition, cursorSavePosition } from "./escapes";
...
export function slice(text: string, start: number | null, end: number | null) {
  ...
  let rStart = start === null ? 0 : start;
  let rEnd = end === null ? 1 : end;
  if (rStart < 0) {
    rStart = textLen + rStart;
  }
  if (rEnd < 0) {
    rEnd = textLen + rEnd;
  }
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    if (eawLen >= rStart - (charLen === 2 ? 1 : 0)) {
      if (eawLen + charLen <= rEnd) {
        result += char;
      } else {
        break;
      }
...
export class TerminalCursor {
  ...
  public write(text: string | Uint8Array, forward: boolean) {
    let payload = typeof text === "string" ? text : new TextDecoder().decode(text);
    if (forward) {
      payload = cursorSavePosition + text + cursorRestorePosition;
    }
...
export function getSharedFragment(fragment: string, candidates: string[]): string {
  ...
  newFragment += candidates[0].slice(newFragment.length, newFragment.length + 1);

  for (let i = 0; i < candidates.length; i++) {
    // this is wrong candidate
    if (!candidates[i].startsWith(fragment)) return "";

    if (!candidates[i].startsWith(newFragment)) return fragment;
  }
